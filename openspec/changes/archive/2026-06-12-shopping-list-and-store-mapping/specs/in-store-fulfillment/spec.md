...
**Reason**: The structured `item_locations` and `doesnt_carry` facets are removed; their role moves to attributed store notes (`location` and `stock` tags), collapsing store layout and store observations into one surface.

**Migration**: Record where-it-hides placements as `location`-tagged store notes and not-carried entries as `stock`-tagged notes via `add_store_note`. Correct or clear them with `update_store_note` / `remove_store_note` (your own notes) or a superseding note (another tenant's). Legacy structured facets in existing `stores/*.toml` are ignored by the parser, not migrated.
