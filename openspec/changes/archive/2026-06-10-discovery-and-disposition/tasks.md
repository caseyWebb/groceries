...
- [x] 7.2b Local MCP smoke (`wrangler dev`, Access gate off): all 3 discovery tools register; `import_recipe` on a live recipe page returns parsed data via the real workerd `HTMLRewriter` path (canonical source, clean ingredients/steps, durations); a no-JSON-LD page → `{ error: "no_jsonld" }`. CD itself ran typecheck + the full vitest suite + deploy (green).
- [ ] 7.2c **Prod manual residual** (Access-gated — needs Casey's authenticated MCP Inspector session against the deployed Worker): `create_recipe(...)` lands a draft `recipes/*.md` in one real commit and rejects a duplicate slug; `fetch_rss_discoveries` dedup excludes an already-imported source. (Natural to fold into the Change 07 connection milestone; `create_recipe` uses the same atomic-commit `gh` client as the already-deployed write tools.)
