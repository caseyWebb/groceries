...
export function registerCookingTools(server: McpServer, gh: GitHubClient): void {
  server.registerTool(
    "read_meal_plan",
    {
      description:
  ...
  server.registerTool(
    ...
    ({ period }) =>
      runTool(async () => {
        const logText = await readOptional(gh, COOKING_LOG_PATH);
        ...
        return retrospective(entries, index, period ?? "month");
      }),
  );
}
