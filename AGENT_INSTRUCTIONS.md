...
1. **Load the selection context up front — one parallel batch, before you settle on recipes.** Call `read_pantry()`, `read_preferences()`, `read_taste()`, `ready_to_eat_available()`, `kroger_flyer()`, **and `list_recipes({ status: "active" })`** together. That single `list_recipes` is the **faceted load**: `course` rides every entry, so one call returns the active **mains and sides together** with full metadata — bucket them by `course` (`main`, `side`, …) yourself. There is **no** separate call later to go hunting for sides; you reason over the mains and sides you already hold. The **raw pantry is a selection input**, not just a post-pick filter: what I already have should pull the menu toward it ("you've got salmon and bok choy — lean into these"), and reasoning over it is also how you spot inventory stand-ins later. Pick (open-ended) or work outward from (recipe-seeded) by reasoning over the loaded recipe metadata **+** the loaded pantry — there is **no** `verify_pantry_*` call. (`kroger_prices` is a *costing* call — it needs the chosen ingredients, so it waits for step 4.)
...
2. **Pantry pass — reason over the loaded pantry, then `mark_pantry_verified(items)` for anything I confirm.** This is your judgment over the pantry you just loaded, not a tool's buckets (the precise ingredient-by-ingredient to-buy match is finalized at step 4, once you've read the chosen recipes). Don't skip staples and spices to save time — they're the category that silently runs out. Specifically:
   - **Freshness is your judgment.** Scan each on-hand item's age metadata (`added_at`, `last_verified_at`, `category`, `prepared_from`) and prompt me about anything that may have drifted — perishables long-unverified, leftovers (`prepared_from`) more than a few days old ("basil verified 9 days ago — still good?"), long-frozen items worth using up ("pork shoulder's been in the freezer 4 months — want to factor it in?"). Nudge the genuinely questionable ones, not every item; if nothing looks off, skip this. And **bias the menu toward using up the soon-to-spoil ones (expiry-matching)**: a waning fridge perishable or an aging leftover is a reason to favor a recipe or side that consumes it — judged from `added_at`/`category` (fridge spoils faster than freezer/pantry), since there's no stored expiry date.
...
3. **Group it — department vs aisle (graceful degradation).**
